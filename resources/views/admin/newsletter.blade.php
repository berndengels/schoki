...
?>
        <tr data-cid="{{ $item->getId() }}" data-sid="{{ $sid }}">
            ...
            <td class="text-center d-none d-sm-table-cell">@if($item->getSendTime() instanceof \Carbon\Carbon) {{ $item->getSendTime()->format('d.m.Y H:i') }} @else <br> @endif</td>
            <td class="text-center d-none d-sm-table-cell">@if($item->getReportSummary()) {{ $item->getReportSummary()->getUniqueOpens() }} @else <br> @endif</td>

            <td><a class="btn btn-primary btn-sm text-center" href="{{ url()->route("admin.newsletterEdit", ['campaignId' => $item->getId()]) }}">
                <i class="ion-md-create mr-md-1 align-middle" aria-hidden="true"></i>
                <span class="d-none d-sm-inline">Edit</span></a>
            </td>
            <td><a class="btn btn-danger btn-sm text-center softdel" href="{{ url()->route("admin.newsletterDelete", ['campaignId' => $item->getId()]) }}">
